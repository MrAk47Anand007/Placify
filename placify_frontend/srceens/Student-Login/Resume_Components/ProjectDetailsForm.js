...
//                   <FontAwesome name="calendar" size={20} color={Colors.primary} />
//                   <Text style={styles.dateText}>{project.startDate instanceof Date ? project.startDate.toLocaleDateString() : 'DD-MM-YYYY'}</Text>
//                 </TouchableOpacity>
//               </View>
//               <View style={styles.dateContainer}>
//                 <Text style={styles.dateLabel}>To:</Text>
...
//                   <FontAwesome name="calendar" size={20} color={Colors.primary} />
//                   <Text style={styles.dateText}>{project.endDate instanceof Date ? project.endDate.toLocaleDateString() : 'DD-MM-YYYY'}</Text>
//                 </TouchableOpacity>
//               </View>
//             </View>
//             <DatePicker
//               modal
//               open={openFromDatePicker}
//               date={projects[selectedDateIndex]?.startDate instanceof Date ? projects[selectedDateIndex].startDate : new Date()}
//               mode="date"
...
//               modal
//               open={openToDatePicker}
//               date={projects[selectedDateIndex]?.endDate instanceof Date ? projects[selectedDateIndex].endDate : new Date()}
//               mode="date"
...
const ProjectDetailsForm = ({ onSubmit }) => {
  ...
  const [projects, setProjects] = useState(
    resumeData.projects ? resumeData.projects : [{
      ...
      endDate: new Date(),
      teamSize: '',
      keySkills: [], // Initialize keySkills as an array
      description: '',
    }]
  );
  ...
  const handleChange = (text, field, index) => {
    ...
    if (field === 'keySkills') {
      updatedProjects[index][field] = text.split(',').map(skill => skill.trim());
    } else {
      updatedProjects[index][field] = text;
    }
  ...
  const handleAddNew = () => {
    setOpenFromDatePicker(false); // Close from date picker
    setOpenToDatePicker(false);
    setProjects([
      ...projects,
      {
        ...
        <View key={index} style={styles.card}>
          ...
          <View style={styles.formContainer}>
            ...
            <TextInput
              style={styles.input}
              placeholder="Key Skills"
              value={Array.isArray(project.keySkills) ? project.keySkills.join(', ') : ''}
              onChangeText={(text) => handleChange(text, 'keySkills', index)}
            />
            <Text>Description</Text>
            <TextInput
