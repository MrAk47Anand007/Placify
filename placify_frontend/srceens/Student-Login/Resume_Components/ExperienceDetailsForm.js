...
//     const updatedExperiences = [...experiences];
//     if (field === 'skills') {
//       updatedExperiences[index][field] = text.split(',').map(skill => skill.trim());
//     } else {
//       updatedExperiences[index][field] = text;
//     }
//     // Ensure skills property is always initialized as an array
//     if (!Array.isArray(updatedExperiences[index].skills)) {
          ...
          <View style={styles.formContainer}>
            ...
            <View style={styles.dateRow}>
              <View style={styles.dateContainer}>
                ...
                <TouchableOpacity style={styles.datePicker} onPress={() => handleOpenDatePicker(index, 'startDate')}>
                  <FontAwesome name="calendar" size={20} color={Colors.primary} />
                  <Text style={styles.dateText}>{experience.startDate instanceof Date ? experience.startDate.toLocaleDateString() : 'DD-MM-YYYY'}</Text>
                </TouchableOpacity>
              </View>
              {!experience.isCurrentlyWorking ? (
                <View style={styles.dateContainer}>
                  ...
                  <TouchableOpacity style={styles.datePicker} onPress={() => handleOpenDatePicker(index, 'endDate')}>
                    <FontAwesome name="calendar" size={20} color={Colors.primary} />
                    <Text style={styles.dateText}>{experience.endDate instanceof Date ? experience.endDate.toLocaleDateString() : 'DD-MM-YYYY'}</Text>
                  </TouchableOpacity>
                </View>
              ) : (
                <Text style={styles.presentText}>To: Present</Text>
            ...
            <DatePicker
              modal
              open={openDatePicker.open && openDatePicker.index === index && openDatePicker.field === 'startDate'}
              date={experience.startDate instanceof Date ? experience.startDate : new Date()}
              mode="date"
              onConfirm={(date) => {
                handleDateChange(date, 'startDate', index);
                handleCloseDatePicker();
            ...
            <DatePicker
              modal
              open={openDatePicker.open && openDatePicker.index === index && openDatePicker.field === 'endDate'}
              date={experience.endDate instanceof Date ? experience.endDate : new Date()}
              mode="date"
              onConfirm={(date) => {
                handleDateChange(date, 'endDate', index);
                handleCloseDatePicker();
            ...
            <TextInput
              style={styles.input}
              placeholder="Skills"
              value={Array.isArray(experience.skills) ? experience.skills.join(', ') : ''}
              onChangeText={(text) => handleChange(text, 'skills', index)}
            />
            <Text>Description</Text>
            <TextInput
