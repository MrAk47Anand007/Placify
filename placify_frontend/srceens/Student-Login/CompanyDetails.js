...
import { StyleSheet, SafeAreaView, ScrollView, View, Text, TouchableOpacity, Dimensions, Image, Animated, Alert } from 'react-native';
import FontAwesome from 'react-native-vector-icons/FontAwesome';
import MaterialCommunityIcons from 'react-native-vector-icons/MaterialCommunityIcons';
import Colors from '../../constants/Colors';
import Spacing from '../../constants/Spacing';
...
const CompanyDetails = () => {
  ...
  useEffect(() => {
    const fetchLogo = async () => {
      const newLogo = await getCompanyLogo(item.companyName);
      setLogo(newLogo);
    };
    fetchLogo();
  }, [item.companyName]);
  ...
  if (showPdf) {
    return <RenderPdf uri={item.fileUrl} />;
  }
  ...
  const handleApply = async () => {
    try {
      ...
      const placementDriveId = item.id;

      console.log(studentId);
      console.log(placementDriveId);
  
      const response = await axios.post(
        `${config.apiEndpoint}/student/apply`,
        null,
        {
          params: {
            studentId,
      ...
      Alert.alert("Error submitting application:", error.response.data.message);
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.headerContainer}>
        ...
        <View>
          <Text style={styles.jobRoleText}>{item.title}</Text>
      ...
      <ScrollView contentContainerStyle={styles.scrollViewContent}>
        <Animated.View style={{ opacity: fadeAnim }}>
          <View style={styles.contentContainer}>
            ...
            <View style={styles.locationContainer}>
              <View style={styles.locationRectangle}>
                <Text style={styles.rectangleText}>{item.employmentType}</Text>
              </View>
            </View>
          </View>
          ...
          <View style={styles.contentContainer}>
            ...
            <View style={styles.locationContainer}>
              <View style={styles.locationRectangle}>
                <Text style={styles.rectangleText}>{item.costToCompany}</Text>
              </View>
            </View>
          </View>
          ...
          <View style={styles.contentContainer}>
            ...
            <View style={styles.locationContainer}>
              {item.location && item.location.split(', ').map((location, index) => (
                <View key={index} style={styles.locationRectangle}>
                  <Text style={styles.locationText}>{location}</Text>
                </View>
              ))}
          ...
          <View style={styles.contentContainer}>
            ...
            <View style={styles.criteriaContainer}>
              <Text style={styles.criteriaTitle}>Current Backlogs:</Text>
              <Text style={styles.criteriaValue}>{item.eligibilityCriteria.allowCurrentBacklogs ? 'Allowed' : 'Not Allowed'}</Text>
            </View>
            <View style={styles.criteriaContainer}>
              <Text style={styles.criteriaTitle}>Gender:</Text>
              <Text style={styles.criteriaValue}>{item.eligibilityCriteria.gender}</Text>
            </View>
            <View style={styles.criteriaContainer}>
              <Text style={styles.criteriaTitle}>Max Previous Backlogs:</Text>
              <Text style={styles.criteriaValue}>{item.eligibilityCriteria.maximumPreviousBacklogs}</Text>
            </View>
            <View style={styles.criteriaContainer}>
              <Text style={styles.criteriaTitle}>Min CGPA:</Text>
              <Text style={styles.criteriaValue}>{item.eligibilityCriteria.minimumCgpa}</Text>
            </View>
            <View style={styles.criteriaContainer}>
              <Text style={styles.criteriaTitle}>Min 10th Marks (%):</Text>
              <Text style={styles.criteriaValue}>{item.eligibilityCriteria.minimumTenthMarks}</Text>
            </View>
            <View style={styles.criteriaContainer}>
              <Text style={styles.criteriaTitle}>Min 12th Marks (%):</Text>
              <Text style={styles.criteriaValue}>{item.eligibilityCriteria.minimumTwelfthMarks}</Text>
            </View>
          </View>
          ...
          <View style={styles.contentContainer}>
            ...
            <View style={styles.processContainer}>
              <Text style={styles.processText}>{item.selectionProcess}</Text>
            </View>
          </View>
          ...
          {item.fileUrl && (
            <View style={styles.contentContainer}>
              <View style={styles.sectionHeader}>
                <MaterialCommunityIcons name="file-document" size={24} color={Colors.text} style={styles.sectionIcon} />
                <Text style={styles.heading}>Necessary Documents</Text>
              </View>
              <TouchableOpacity
